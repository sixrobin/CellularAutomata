namespace CellularAutomata
{
    ...
    public class CellularAutomaton3DGPU : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            (int surviveRule, int birthRule, int cellStatesRule, int neighbourhoodRule) =
                CellularAutomata.RulesParser.ParseRuleset(this._rules, log: true);
        ...
        private void Start()
        {
            this._computeShader.SetFloat("_InitRandomStep",
                this._initializationMethod.HasFlag(E_InitializationMethod.RANDOM_STEP) ? this._initRandomStep : 1f);
            this._computeShader.SetInt("_InitCenterWidth",
                this._initializationMethod.HasFlag(E_InitializationMethod.CENTER_CELL)
                    ? this._initCenterWidth
                    : this._resolution);
